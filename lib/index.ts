...
export type EndpointDefinition<Body, Response, Query = unknown, Params extends Record<string, unknown> = {}> = {
    body: Body,
    response: Response,
    query: Query,
    params: Params
...
export function defineHandler<T extends Endpoint<any>>(definition: T, controller: Controller<T['_type']>): ControllerMetadata<T['_type']> {
    return {
        endpoint: definition,
...
export function defineEndpoint<Body, Response, Query = never, Params extends Record<string, unknown> = {}>(method: HttpMethod, path: string): Endpoint<EndpointDefinition<Body, Response, Query, Params>> {
    return {
        method, path,
